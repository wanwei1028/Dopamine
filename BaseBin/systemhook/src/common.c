...
kSpawnConfig spawn_config_for_executable(const char* path, char *const argv[restrict])
{
	...
	if (access(jectPath, F_OK) == 0)
	{
		...
		if (exec && wantInject(exec + 1, jectPath)) return (kSpawnConfigInject | kSpawnConfigTrust);
		...
		*/
		...
		{
			if (strstr(path, whitelist[i])) return (kSpawnConfigInject | kSpawnConfigTrust);
		}
	...
	if (shouldInsertJBEnv) {
		...
		if (attrStruct) {
			...
			if (jetsamMultiplier > 1) {
				int wantJetsamMultiplier = (int)round(jetsamMultiplier * 3) + (strstr(path, "/.jbroot-") ? 8 : 0);
